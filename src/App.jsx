...
const LOW_STOCK_THRESHOLD = 0.2; // warn when stock <= 20% of capacity
...
// Parse a rate string like "1.5L/ha" or "5kg/ha" → { amount: 1.5, unit: "L" }
...
function rowToChem(r) {
  return {
    ...
    methods: r.methods || [], resprayWeeks: r.respray_weeks, autoRespray: r.auto_respray,
  };
}
...
function rowToStock(r) {
  return {
    id: r.id, chemicalName: r.chemical_name, unit: r.unit,
    currentStock: parseFloat(r.current_stock), capacity: parseFloat(r.capacity),
    lowStockThreshold: parseFloat(r.low_stock_threshold),
  };
}
function stockToRow(s) {
  return {
    chemical_name: s.chemicalName, unit: s.unit,
    current_stock: s.currentStock, capacity: s.capacity,
...
// ─── STOCK VIEW ───────────────────────────────────────────────────────────────
function StockView({ stocks, chemicals, onAdd, onUpdate, onDelete, onAdjust, isManager }) {
  const [showForm, setShowForm] = useState(false);
  const [editing, setEditing] = useState(null);
  const [adjusting, setAdjusting] = useState(null); // stock item being topped up
      ...
      <div className="stock-grid">
        {stocks.length === 0 && (
          <div className="card" style={{ padding: 40, textAlign: "center", color: "var(--mid)", gridColumn: "1/-1" }}>
            No stock entries yet. Add chemicals to track stock levels.
          </div>
        )}
        {stocks.map(s => (
      ...
      {adjusting && (
        <Modal title={`Top Up — ${adjusting.chemicalName}`} onClose={() => setAdjusting(null)}>
          <div className="info-box">
            Current stock: <strong>{adjusting.currentStock} {adjusting.unit}</strong> of {adjusting.capacity} {adjusting.unit}
          </div>
          <div className="form-group">
            <label className="form-label">Amount to Add ({adjusting.unit})</label>
            <input type="number" className="form-control" min={0} value={adjustQty}
              onChange={e => setAdjustQty(e.target.value)} placeholder={`e.g. 5`} autoFocus />
          </div>
          <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
            <button type="button" className="btn btn-primary" onClick={() => handleAdjust("add")} disabled={saving}>
              {saving ? "Saving…" : "Add to Stock"}
            </button>
            <button type="button" className="btn btn-ghost" onClick={() => handleAdjust("set")} disabled={saving}>
              Set as Exact Amount
            </button>
        ...
        <Modal title={editing ? "Edit Stock Entry" : "Add Chemical to Stock"} onClose={() => { setShowForm(false); setEditing(null); }}
          ...
          <div className="form-row-3">
            <div className="form-group">
              ...
              <input type="number" min={0} className="form-control" value={form.currentStock}
            ...
            <div className="form-group">
              ...
              <input type="number" min={0} className="form-control" value={form.capacity}
                onChange={e => setForm(f => ({ ...f, capacity: parseFloat(e.target.value) || 0 }))} />
            </div>
            <div className="form-group">
              <label className="form-label">Low Stock Alert at</label>
              <input type="number" min={0} className="form-control" value={form.lowStockThreshold}
                onChange={e => setForm(f => ({ ...f, lowStockThreshold: parseFloat(e.target.value) || 0 }))} />
            </div>
          </div>
          <div className="info-box" style={{ marginTop: 4 }}>
            A warning will show when stock drops to or below the <strong>Low Stock Alert</strong> value.
          </div>
        </Modal>
      )}
    </div>
...
function TodoView({ tasks, chemicals, operatives, stocks, onAdd, onUpdate, onDelete, onStockDeduct, isManager, error }) {
  ...
  async function handleToggle(task) {
    ...
    const updated = { ...task, completed: nowCompleting, completedDate: nowCompleting ? today : null };
    await onUpdate(updated);
    // Deduct stock when marking complete
    if (nowCompleting && task.chemicals?.length > 0) {
      await onStockDeduct(task.chemicals);
    }
  }
  ...
  // Low stock warnings relevant to pending tasks
    ...
    <div>
      ...
      {pendingWithLowStock.length > 0 && (
        <div className="warn-box">
          ⚠ <strong>Low stock warning:</strong> Some pending tasks use chemicals that are running low.
          Check the <strong>Stock</strong> page to top up before spraying.
        </div>
      )}
...
export default function App() {
  ...
  const deductStock = useCallback(async (taskChemicals) => {
    // taskChemicals: [{ name, rate, method }]
    for (const tc of taskChemicals) {
      const parsed = parseRate(tc.rate);
      if (!parsed) continue;
      const stockItem = stocks.find(s => s.chemicalName === tc.name && s.unit === parsed.unit);
      if (!stockItem) continue;
      const newStock = Math.max(0, stockItem.currentStock - parsed.amount);
      ...
      <div className="app">
        <aside className="sidebar">
          ...
          <div className="sidebar-week">
            <span className={`sync-dot${syncStatus === "error" ? " error" : ""}`} title={syncStatus === "ok" ? "Connected" : "Connection error"} />
