...
// Parse "1.5L/ha" → { amount: 1.5, unit: "L" }
...
function rowToTask(r) {
  return {
    id: r.id, location: r.location, instruction: r.instruction || "",
    comments: r.comments || "", operative: r.operative, date: r.date,
    completed: r.completed, completedDate: r.completed_date || null,
    spotSpray: r.spot_spray, frequencyWeeks: r.frequency_weeks,
    nextDate: r.next_date || null, chemicals: r.chemicals || [],
  };
}
function taskToRow(t) {
  return {
    location: t.location, instruction: t.instruction, comments: t.comments,
    operative: t.operative, date: t.date, completed: t.completed,
    completed_date: t.completedDate || null, spot_spray: t.spotSpray,
    frequency_weeks: t.frequencyWeeks, next_date: t.nextDate || null,
...
function rowToStock(r) {
  return { id: r.id, chemicalName: r.chemical_name, unit: r.unit, currentStock: parseFloat(r.current_stock), capacity: parseFloat(r.capacity), lowStockThreshold: parseFloat(r.low_stock_threshold) };
}
function stockToRow(s) {
  return { chemical_name: s.chemicalName, unit: s.unit, current_stock: s.currentStock, capacity: s.capacity, low_stock_threshold: s.lowStockThreshold };
}
...
// Shown when a task is ticked complete — asks how much of each chemical was used
function UsageModal({ task, stocks, onConfirm, onSkip, onCancel, saving }) {
  // Build initial usage entries from task chemicals
  const initialUsage = task.chemicals.map(c => {
    ...
    return {
      ...
      unit: stockItem?.unit || (parsed?.unit || "L"),
  ...
  function updateAmount(i, val) {
    setUsage(u => u.map((x, idx) => idx === i ? { ...x, amount: val } : x));
  }

  return (
    <Modal title="How much was used?" onClose={onCancel}
      footer={
        <div style={{ display: "flex", gap: 10, justifyContent: "space-between", width: "100%" }}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <button type="button" className="btn btn-primary" onClick={() => onConfirm(usage)} disabled={saving}>
      ...
      <div className="info-box">
        Enter the actual amount of each chemical used. This will be deducted from your stock levels.
      </div>
      {usage.map((u, i) => (
        <div key={i} className="usage-chem-row">
          ...
          {u.stockItem ? (
            <>
              <div className="usage-chem-meta">
                Current stock: {u.stockItem.currentStock} {u.stockItem.unit}
              </div>
              ...
              {u.amount !== "" && parseFloat(u.amount) > u.stockItem.currentStock && (
                <div style={{ color: "var(--red)", fontSize: 12, marginTop: 6 }}>
                  ⚠ Amount exceeds current stock ({u.stockItem.currentStock} {u.unit})
                </div>
              )}
            </>
          ) : (
            <div className="usage-no-stock">
              No stock entry found for this chemical — usage will not be recorded.
              Add it on the Stock page to track it in future.
            </div>
          )}
        </div>
      ))}
...
function TodoView({ tasks, chemicals, operatives, stocks, onAdd, onUpdate, onDelete, onStockDeduct, isManager, error }) {
  ...
  // Task awaiting usage confirmation before being marked complete
  ...
  // - If marking complete → show usage modal
  // - If un-completing → just update directly
  function handleToggle(task) {
    if (task.completed) {
      // Un-completing — no usage dialog needed
      onUpdate({ ...task, completed: false, completedDate: null });
    } else {
      // About to complete — show usage modal
      setPendingComplete(task);
    }
  }

  // User confirmed usage amounts — deduct and complete
  async function handleUsageConfirm(usage) {
    setUsageSaving(true);
    try {
      const updated = { ...pendingComplete, completed: true, completedDate: today };
      await onUpdate(updated);
      // Deduct only entries that have a stockItem and a valid amount
      ...
      await onStockDeduct(deductions);
      setPendingComplete(null);
    } finally { setUsageSaving(false); }
  }

  // User chose to complete without recording usage
  async function handleUsageSkip() {
    setUsageSaving(true);
    try {
  ...
  return (
    <div>
      ...
      {pendingWithLowStock.length > 0 && (
        <div className="warn-box">
          ⚠ <strong>Low stock warning:</strong> Some pending tasks use chemicals that are running low — check the Stock page before spraying.
        </div>
      )}
              ...
              {sorted.map(task => (
                <tr key={task.id} className={rowClass(task)}>
                  ...
                  <td style={{ minWidth: 200 }}>
                    ...
                    {!task.completed && task.chemicals?.some(c => lowStockNames.has(c.name)) && (
                      <div style={{ marginTop: 4 }}><span className="badge badge-low">⚠ Low stock</span></div>
                    )}
                  </td>
...
function StockView({ stocks, chemicals, onAdd, onUpdate, onDelete, isManager }) {
  ...
  function openEdit(s) { setForm({ ...s }); setEditing(s); setShowForm(true); }
    ...
    <div>
      {lowItems.length > 0 && (
        <div className="warn-box" style={{ marginBottom: 20 }}>
          ⚠ <strong>Low stock alert:</strong> {lowItems.map(s => `${s.chemicalName} (${s.currentStock}${s.unit} remaining)`).join(" · ")}
        </div>
      )}
      <div className="stock-grid">
        {stocks.length === 0 && (
          <div className="card" style={{ padding: 40, textAlign: "center", color: "var(--mid)", gridColumn: "1/-1" }}>
        ...
        {stocks.map(s => (
          <div key={s.id} className={cardClass(s)}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              <div>
                <div className="stock-card-name">{s.chemicalName}</div>
                <div className="stock-card-unit">measured in {s.unit}</div>
              </div>
              {stockPct(s) <= 25 && (
                <span className={`badge ${stockPct(s) <= 10 ? "badge-low" : "badge-spot"}`}>
                  {stockPct(s) <= 10 ? "⚠ Critical" : "Low stock"}
                </span>
              )}
            </div>
            <div className="stock-card-nums">
              <span className="stock-card-current" style={{ color: stockPct(s) <= 10 ? "var(--red)" : stockPct(s) <= 25 ? "var(--gold)" : "var(--forest)" }}>
                {s.currentStock}
              </span>
            ...
            <div style={{ display: "flex", gap: 6, marginTop: 12 }}>
              ...
              {isManager && <button type="button" className="btn btn-ghost btn-sm" onClick={() => openEdit(s)}>✏️ Edit</button>}
              {isManager && <button type="button" className="btn btn-danger btn-sm" onClick={() => { if (confirm("Remove this stock entry?")) onDelete(s.id); }}>🗑</button>}
            </div>
          </div>
        ))}
      </div>
      {isManager && (
        <button type="button" className="btn btn-primary" onClick={() => { setForm(blank); setEditing(null); setShowForm(true); }}>
          + Add Chemical to Stock List
        </button>
      )}
      ...
      {adjusting && (
        <Modal title={`Top Up — ${adjusting.chemicalName}`} onClose={() => setAdjusting(null)}>
          ...
          <div className="form-group">
            <label className="form-label">Amount to Add ({adjusting.unit})</label>
            <input type="number" className="form-control" min={0} value={adjustQty} onChange={e => setAdjustQty(e.target.value)} placeholder="e.g. 5" autoFocus />
          </div>
        ...
        <Modal title={editing ? "Edit Stock Entry" : "Add Chemical to Stock"} onClose={() => { setShowForm(false); setEditing(null); }}
          footer={
            <>
              ...
              <button type="button" className="btn btn-primary" onClick={handleSave} disabled={saving}>{saving ? "Saving…" : editing ? "Save" : "Add to Stock"}</button>
            </>
          }>
          <div className="form-row">
            <div className="form-group">
              <label className="form-label">Chemical *</label>
              <select className="form-control" value={form.chemicalName} onChange={e => setForm(f => ({ ...f, chemicalName: e.target.value }))}>
          ...
          <div className="form-row-3">
            <div className="form-group">
              ...
              <input type="number" min={0} className="form-control" value={form.currentStock} onChange={e => setForm(f => ({ ...f, currentStock: parseFloat(e.target.value) || 0 }))} />
            </div>
            <div className="form-group">
              <label className="form-label">Max Capacity</label>
              <input type="number" min={0} className="form-control" value={form.capacity} onChange={e => setForm(f => ({ ...f, capacity: parseFloat(e.target.value) || 0 }))} />
            </div>
            <div className="form-group">
              <label className="form-label">Low Stock Alert at</label>
              <input type="number" min={0} className="form-control" value={form.lowStockThreshold} onChange={e => setForm(f => ({ ...f, lowStockThreshold: parseFloat(e.target.value) || 0 }))} />
            </div>
          </div>
...
export default function App() {
  ...
  // ── Stock deduction — receives [{ stockItem, amount }] ────────
  ...
  // ── Stock ─────────────────────────────────────────────────────
  const addStock = useCallback(async (form) => {
    const rows = await sbInsert("stock", stockToRow(form));
    setStocks(ss => [...ss, rowToStock(rows[0])]);
  }, []);
  const updateStock = useCallback(async (s) => {
    await sbUpdate("stock", s.id, stockToRow(s));
    setStocks(ss => ss.map(x => x.id === s.id ? s : x));
  }, []);
  const deleteStock = useCallback(async (id) => {
    await sbDelete("stock", id);
    setStocks(ss => ss.filter(x => x.id !== id));
  }, []);
      ...
      <div className="app">
        ...
        <div className="main">
          ...
          <div className="content">
            {!isManager && !["todo", "schedule", "stock"].includes(view) && (
